...
<%
	boolean login;
	String str = (String)session.getAttribute("adminLogined"); //是否登陆
	login = (str == null || !str.trim().equals("true"))?false:true;
	...
	sql = "select count(*) from daomu where pid = 0";
	...
	sql = "select * from daomu where pid = 0 order by pdate desc limit "+(pageNo-1)*PAGE_SIZE+","+PAGE_SIZE;
	rs = DB.getResult(stmt, sql);
...
<%--删除帖子： --%>	 <a href="delete.jsp?id=<%=a.getId()%>&isLeaf=<%=a.isLeaf()%>&pid=<%=a.getPid()%>">Del</a>
			...
			<%--帖子内容： --%>												<a href="articleDetailFlat.jsp?id=<%=a.getId() %>" target="_blank" title="<%= a.getCont()%>"><%= a.getCont()%></a>
...
<div class="pagebox">
 <ul>
 <li><a href="articleFlat.jsp?pageNo=<%= pageNo-1%>">上一页</a></li> <li><a class="abg">第<%= pageNo%>页,共<%= pages%>页</a></li> <li><a href="articleFlat.jsp?pageNo=<%= pageNo+1%>" class="next">下一页</a></li>  </ul>
</div>
